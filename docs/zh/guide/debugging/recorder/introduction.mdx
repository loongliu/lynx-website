...
Recorder 设计目标之一就是屏蔽不同应用与 Lynx 数据通信的差异，从而实现 Lynx 页面的跨应用、跨设备乃至跨平台回放。

录制阶段，Recorder 模块会悄悄的记录下你的应用与 Lynx 之间所有的数据通信序列，这些记录信息最终会被打包成录制产物。

回放阶段，Recorder 借助于录制阶段记录的数据，自己承担起应用的角色与 Lynx 进行数据通信。

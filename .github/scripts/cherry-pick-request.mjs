...
function renderSummary({
  ...
  const body = [
    ...
    sourceTitle ? `- Source PR title: ${sourceTitle}` : '',
    ...
    '### Reason',
    '',
    reason ? reason : '_No reason parsed yet._',
    '',
    '### Targets',
    '',
...
function prHasGeneratedIdentity(
  ...
  const hasMarker =
    ...
    (body.includes(GENERATED_MARKER_PREFIX) &&
      body.includes(`source-pr=${sourcePr}`) &&
  ...
  return (
    pr.base?.ref === target &&
    (hasMarker || headMatches || (labelMatches && hasMarker))
  );
}
...
async function findExistingGeneratedPr(
  ...
  const open = openPulls.find((pr) =>
    prHasGeneratedIdentity(pr, sourcePr, target, requestIssue, branchName),
  );
  ...
  const marker = generatedMarker(sourcePr, target, requestIssue);
  const markerOpen = (await listPulls(repo, 'open')).find(
    (pr) => pr.base?.ref === target && String(pr.body || '').includes(marker),
  );
  if (markerOpen) return { kind: 'open', pr: markerOpen };

  const markerClosed = (await listPulls(repo, 'closed')).find(
    (pr) => pr.base?.ref === target && String(pr.body || '').includes(marker),
  );
  if (markerClosed) {
    return {
      kind: markerClosed.merged_at ? 'merged' : 'closed-unmerged',
      pr: markerClosed,
    };
  }
